...
MDX プロセッサ (Sätteri) が frontmatter を YAML としてパースし、プラグイン側で Eleventy の 11ty.js テンプレートに `export const data = ...` として引き渡します。`data` を直接読み出す仕組みなので、ページ側からは Eleventy 標準の frontmatter と同じ感覚で扱えます。

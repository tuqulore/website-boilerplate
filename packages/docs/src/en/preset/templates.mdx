...
Markdown-style `---` frontmatter conflicts with MDX's JSX parsing, so the boilerplate standardizes on `export const data`. `.jsx` files use the same shape for the same reason.

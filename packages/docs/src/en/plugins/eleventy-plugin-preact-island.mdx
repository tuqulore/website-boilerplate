...
The `on` prop is typed as `IslandOn = "interaction" | "visible" | "idle" | (string & {})`. Editor autocompletion shows the three primary values above; the `(string & {})` branch keeps the escape hatch open, so `<Island on="load" ... />` and `<Island on="save-data" ... />` also type-check and are forwarded to is-land as-is. The runtime accepts anything matching `/^[A-Za-z0-9_-]+$/`.
